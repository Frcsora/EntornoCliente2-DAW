...
form.addEventListener('submit', (event) => {
    ...
    fetch("verPartida.php", {
        method: 'POST',
        headers: {'Content-type': 'application/json'},
        body: JSON.stringify({id: document.getElementById("select").value})
    })
...
tablero.addEventListener("click", (event) => {
    let seleccion = event.target.closest(".casillas");
    ...
    }else if(!seleccion.hasChildNodes()){
        fin= false;
        movimientos.forEach((movimiento) => {
            const abc = "abcdefgh"
            const letra1 = movimiento.movimiento.charAt(1);
            const numero1 = movimiento.movimiento.charAt(2);
            const letra2 = movimiento.movimiento.charAt(3);
            const numero2 = movimiento.movimiento.charAt(4);
            turnos.push(abc.indexOf(letra1) + "" + numero1 - 1);
            turnos.push(abc.indexOf(letra2) + "" + numero2 - 1);
        })
        console.log(turnos)
        repeticion();
    }
...
function repeticion(){
    intervalo = setInterval(()=>{
        let casilla = document.getElementById("casilla" + turnos[turnoActual]);
        casilla.click();
        turnoActual++;
    }, 500)
}
